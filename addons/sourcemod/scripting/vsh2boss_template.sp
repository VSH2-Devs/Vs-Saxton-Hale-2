...
public void OnLibraryAdded(const char[] name) {
	if( StrEqual(name, "VSH2") ) {
		...
		template_boss.cfg.Get("plugin name", plugin_name_str, sizeof(plugin_name_str));
...
stock bool IsTemplate(VSH2Player player) {
...
public void Template_OnCallDownloads() {
	{
		/// model.
		int boss_mdl_len = template_boss.cfg.GetSize("model");
		char[] boss_mdl_str = new char[boss_mdl_len];
		if( template_boss.cfg.Get("model", boss_mdl_str, boss_mdl_len) > 0 ) {
			PrepareModel(boss_mdl_str);
		}
		
		/// model skins.
		ConfigMap skins = template_boss.cfg.GetSection("skins");
		PrepareAssetsFromCfgMap(skins, ResourceMaterial);
	}
	
	ConfigMap sounds_sect = template_boss.cfg.GetSection("sounds");
	if( sounds_sect != null ) {
		int num_sound_sects = sounds_sect.Size;
		for( int i; i < num_sound_sects; i++ ) {
			int sect_len = sounds_sect.GetKeySize(i);
			char[] sect_name = new char[sect_len + 1];
			sounds_sect.GetKey(i, sect_name, sect_len);
			PrepareAssetsFromCfgMap(sounds_sect.GetSection(sect_name), ResourceSound);
		}
	}
}
...
public void Template_OnBossMenu(Menu& menu) {
	...
	int menu_name_len = template_boss.cfg.GetSize("menu name");
	char[] menu_name_str = new char[menu_name_len];
	template_boss.cfg.Get("menu name", menu_name_str, menu_name_len);
	menu.AddItem(tostr, menu_name_str);
}

public void Template_OnBossSelected(VSH2Player player) {
	...
	ConfigMap custom_props = template_boss.cfg.GetSection("custom props");
	...
	int panel_len = template_boss.cfg.GetSize("help panel");
	char[] panel_info = new char[panel_len];
	template_boss.cfg.Get("help panel", panel_info, panel_len);
...
public void Template_OnBossThink(VSH2Player player) {
	...
	if( player.SuperJumpThink(2.5, 25.0) ) {
		player.PlayRandVoiceClipCfgMap(template_boss.cfg.GetSection("sounds.jump"), VSH2_VOICE_ABILITY);
...
public void Template_OnBossHUD(VSH2Player player, char hud_text[PLAYER_HUD_SIZE]) {
	/// custom HUD stuff here!
	/// basic stuff like rage or superjump are handled internally by OnBossHUD.
}

public void Template_OnBossModelTimer(VSH2Player player) {
	if( !IsTemplate(player) ) {
		return;
	}
	...
	template_boss.cfg.Get("model", boss_mdl, boss_mdl_len);
	SetVariantString(boss_mdl);
	AcceptEntityInput(client, "SetCustomModel");
	SetEntProp(client, Prop_Send, "m_bUseClassAnimations", 1);
}

public void Template_OnBossEquipped(VSH2Player player) {
	if( !IsTemplate(player) ) {
		return;
	}
	
	char name[MAX_BOSS_NAME_SIZE];
	template_boss.cfg.Get("name", name, sizeof(name));
	player.SetName(name);
	
	player.RemoveAllItems();
	ConfigMap melee_wep = template_boss.cfg.GetSection("melee");
	if( melee_wep==null ) {
		return;
	}
	...
	ConfigMap abilities_sect = template_boss.cfg.GetSection("abilities");
	...
	for( int i; i < num_abilities; i++ ) {
		...
		player.GiveAbility(key);
	}
}


public void Template_OnBossInitialized(VSH2Player player) {
	if( !IsTemplate(player) ) {
		return;
	}
...
public void Template_OnBossPlayIntro(VSH2Player player) {
	if( !IsTemplate(player) ) {
		return;
	}
	player.PlayRandVoiceClipCfgMap(template_boss.cfg.GetSection("sounds.intro"), VSH2_VOICE_INTRO);
}

public void Template_OnPlayerKilled(VSH2Player attacker, VSH2Player victim, Event event) {
	...
	attacker.PlayRandVoiceClipCfgMap(template_boss.cfg.GetSection("sounds.kill"), VSH2_VOICE_SPREE);
	if( attacker.GetPropInt("iKills") == 3 && template_boss.gm.iLivingReds != 1 ) {
		attacker.PlayRandVoiceClipCfgMap(template_boss.cfg.GetSection("sounds.spree"), VSH2_VOICE_SPREE);
		attacker.SetPropInt("iKills", 0);
	} else {
...
public void Template_OnPlayerHurt(VSH2Player attacker, VSH2Player victim, Event event) {
...
public void Template_OnPlayerAirblasted(VSH2Player airblaster, VSH2Player airblasted, Event event) {
...
public void Template_OnBossMedicCall(VSH2Player player) {
	...
	player.PlayRandVoiceClipCfgMap(template_boss.cfg.GetSection("sounds.rage"), VSH2_VOICE_RAGE);
	player.SetPropFloat("flRAGE", 0.0);
}

public void Template_OnBossJarated(VSH2Player victim, VSH2Player thrower) {
...
public void Template_OnRoundEndInfo(VSH2Player player, bool boss_won, char message[MAXMESSAGE]) {
	if( !IsTemplate(player) ) {
		return;
	} else if( boss_won ) {
		player.PlayRandVoiceClipCfgMap(template_boss.cfg.GetSection("sounds.win"), VSH2_VOICE_WIN);
	}
}


public void Template_Music(char song[PLATFORM_MAX_PATH], float& time, VSH2Player player, float& volume) {
	if( !IsTemplate(player) ) {
		return;
	}
	
	ConfigMap music_sect = template_boss.cfg.GetSection("sounds.music");
	if( music_sect==null ) {
		return;
	}
...
public void Template_OnBossDeath(VSH2Player player) {
	if( !IsTemplate(player) ) {
		return;
	}
	
	player.PlayRandVoiceClipCfgMap(template_boss.cfg.GetSection("sounds.death"), VSH2_VOICE_LOSE);
}
...
public Action Template_OnStabbed(VSH2Player victim, int& attacker, int& inflictor, float& damage, int& damagetype, int& weapon, float damageForce[3], float damagePosition[3], int damagecustom) {
	...
	victim.PlayRandVoiceClipCfgMap(template_boss.cfg.GetSection("sounds.backstab"), VSH2_VOICE_STABBED);
	return Plugin_Continue;
}

public void Template_OnLastPlayer(VSH2Player player) {
	if( !IsTemplate(player) ) {
		return;
	}
	player.PlayRandVoiceClipCfgMap(template_boss.cfg.GetSection("sounds.lastplayer"), VSH2_VOICE_LASTGUY);
}

public Action Template_OnSoundHook(VSH2Player player, char sample[PLATFORM_MAX_PATH], int& channel, float& volume, int& level, int& pitch, int& flags) {
	if( !IsTemplate(player) ) {
		return Plugin_Continue;
...
stock void SetPawnTimerEx(Function func, float thinktime=0.1, const any[] args, int len) {
