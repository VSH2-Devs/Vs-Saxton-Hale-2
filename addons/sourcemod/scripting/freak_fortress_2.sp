...
public any Native_FF2_GetBossSpecial(Handle plugin, int numParams)
{
	int boss = GetNativeCell(1);
	int buffer_len = GetNativeCell(3);
	int meaning = GetNativeCell(4);
	...
	if( meaning==0 ) {
		if( boss < 0 || boss > MaxClients )
			...
			if( amount_of_bosses > 0 ) {
				...
				if( players[boss].GetName(name) ) {
					SetNativeString(2, name, MAX_BOSS_NAME_SIZE);
					return true;
				}
				return false;
			}
