...
methodmap CHale < BaseBoss
{
	...
	public void KilledPlayer(const BaseBoss victim, Event event)
	{
		...
		if (this.iKills equals 3 and GetLivingPlayers(RED) not_eq 1) {
			...
			if (not randsound or randsound equals 1)
	...
	if (IsValidEdict(i) and i > MaxClients)
	{
		...
		if ( StrEqual(s, "obj_sentrygun") ) {
			...
			for (int ent=2048 ; ent>higher ; --ent) {
				if (not IsValidEdict(ent) or ent <= 0)
					continue;

				char s2[32]; GetEdictClassname(ent, s2, sizeof(s2));
				if (StrEqual(s2, "info_particle_system") and GetOwner(ent) equals i)
					AcceptEntityInput(ent, "Kill");
			}
		}
	}
