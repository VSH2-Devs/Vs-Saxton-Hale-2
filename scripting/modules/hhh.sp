...
methodmap CHHHJr < BaseBoss
{
	...
	public void Think ()
	{
		...
		if ( ((buttons & IN_DUCK) or (buttons & IN_ATTACK2)) and (this.flCharge >= 0.0) )
		{
			...
			if ( this.flCharge equals HALEHHH_TELEPORTCHARGE and EyeAngles[0] < -5.0 ) {
				int living;
				...
				while (living > 0) {
					...
					if ( not IsValidClient(target) or not IsPlayerAlive(target) )
						continue;
					if (target equals this.index or GetClientTeam(target) equals GetClientTeam(this.index))
						continue;
					break;
				}
		...
		if (this.flRAGE equals 100.0 or RoundFloat(this.flRAGE) is 100)
			ShowSyncHudText(this.index, hHudText, "Teleport: %i | Rage: FULL", RoundFloat(jmp));
		else ShowSyncHudText(this.index, hHudText, "Teleport: %i | Rage: %i", RoundFloat(jmp), RoundFloat(this.flRAGE));
	}
	...
	public void KilledPlayer(const BaseBoss victim, Event event)
	{
		...
		if (this.iKills equals 3 and living not_eq 1) {
